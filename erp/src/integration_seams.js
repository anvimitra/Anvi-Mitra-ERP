...
function registerIntegrationSeamRoutes(app, pool) {
  app.get('/api/integrations/status', authenticate, requireRoles(...adminRoles), async (req, res, next) => {
    try {
      ...
      res.json({
        integrations: {
          payment: { provider: process.env.PAYMENT_PROVIDER || 'manual_adapter', configured: configured('payment', ['PAYMENT_PROVIDER', 'PAYMENT_WEBHOOK_SECRET']) },
          push: { provider: process.env.PUSH_PROVIDER || 'placeholder', configured: configured('push', ['PUSH_PROVIDER', 'PUSH_PROVIDER_KEY']) },
          sms: { provider: process.env.SMS_PROVIDER || 'placeholder', configured: configured('sms', ['SMS_PROVIDER', 'SMS_API_KEY']) },
          email: { provider: process.env.EMAIL_PROVIDER || 'placeholder', configured: configured('email', ['EMAIL_PROVIDER', 'EMAIL_API_KEY']) },
        ...
        recentPaymentIntents: rows.rows,
      });
    } catch (err) { next(err); }
  });
