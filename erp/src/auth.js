...
function registerAuthRoutes(app, pool) {
  app.post('/api/auth/login', async (req,res,next)=>{
    try {
      ...
      if(!login||!password||!schoolCode) return res.status(400).json({error:'login, password and schoolCode are required'});
      const result=await pool.query(`SELECT u.id,u.school_id,u.branch_id,u.password_hash,u.role,u.status,s.name school_name,s.code school_code FROM users u JOIN schools s ON s.id=u.school_id WHERE s.code=$1 AND (lower(u.email)=lower($2) OR u.phone=$2) LIMIT 1`,[String(schoolCode).trim(),normalizeLogin(login)]);
      ...
      const token=signAccessToken({sub:user.id,schoolId:user.school_id,branchId:effectiveBranchId,role:user.role});
      await pool.query('INSERT INTO audit_logs (school_id,user_id,action,entity_type) VALUES ($1,$2,$3,$4)',[user.school_id,user.id,'login','user']);
      res.json({accessToken:token,user:{id:user.id,schoolId:user.school_id,schoolName:user.school_name,branchId:effectiveBranchId,role:user.role}})
    }catch(err){next(err)}
  });
