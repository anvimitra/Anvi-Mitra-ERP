...
function registerFeeAssignmentRoutes(app, pool) {
  ...
  app.get('/api/fees/installments', authenticate, requireRoles(...financeRoles), async(req,res,next)=>{
    try{
      ...
      const {rows}=await pool.query(`SELECT fi.id,fi.student_id AS "studentId",s.admission_no AS "admissionNo",s.full_name AS "studentName",fi.session_id AS "sessionId",fi.fee_head_id AS "feeHeadId",fh.name AS "feeHeadName",fi.period_start AS "periodStart",fi.period_end AS "periodEnd",fi.due_date AS "dueDate",fi.amount,COALESCE(fi.paid_amount,0) AS "paidAmount",GREATEST(0,fi.amount-COALESCE(fi.paid_amount,0)) AS "remainingAmount",fi.invoice_id AS "invoiceId",fi.status FROM fee_installments fi JOIN students s ON s.id=fi.student_id AND s.school_id=fi.school_id JOIN fee_heads fh ON fh.id=fi.fee_head_id AND fh.school_id=fi.school_id WHERE fi.school_id=$1 AND ($2::uuid IS NULL OR fi.session_id=$2) AND ($3::uuid IS NULL OR COALESCE(fi.branch_id,s.branch_id)=$3 OR s.branch_id IS NULL) AND ($4::uuid IS NULL OR fi.student_id=$4) AND ($5::text IS NULL OR fi.status=$5) ORDER BY fi.due_date,s.full_name`,params);
      res.json({installments:rows,totalDue:rows.filter(x=>x.status!=='paid'&&x.status!=='cancelled').reduce((s,x)=>s+Number(x.remainingAmount||0),0)});
    }catch(err){next(err)}
  });
}
