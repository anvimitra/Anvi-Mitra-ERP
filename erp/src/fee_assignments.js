...
const { queueFeeInvoiceNotifications } = require('./notifications');
...
function registerFeeAssignmentRoutes(app, pool) {
  ...
  app.post('/api/fees/installments/generate', authenticate, requireRoles(...financeRoles), async(req,res,next)=>{
    ...
    try{
      ...
      res.status(201).json({generated:created,assignments:assignments.length});
  ...
  app.post('/api/fees/installments/invoice', authenticate, requireRoles(...financeRoles), async(req,res,next)=>{
    ...
    try{
      ...
      let sql=`SELECT fi.id,fi.student_id AS "studentId",fi.fee_head_id AS "feeHeadId",fh.name AS "feeHeadName",fi.period_start AS "periodStart",fi.period_end AS "periodEnd",fi.due_date AS "dueDate",fi.amount,fi.invoice_id AS "invoiceId",COALESCE(fi.branch_id,s.branch_id) AS "branchId" FROM fee_installments fi JOIN fee_heads fh ON fh.id=fi.fee_head_id AND fh.school_id=fi.school_id JOIN students s ON s.id=fi.student_id AND s.school_id=fi.school_id WHERE fi.school_id=$1 AND fi.session_id=$2 AND fi.status='due' AND fi.invoice_id IS NULL AND ($3::uuid IS NULL OR fi.branch_id=$3 OR s.branch_id=$3 OR (fi.branch_id IS NULL AND s.branch_id IS NULL)) AND ($4::uuid IS NULL OR fi.student_id=$4)`;
      ...
      for(const [sid,items] of groups){
        ...
        const branch=items[0].branchId||req.auth.branchId||null;
        ...
        for(const item of items){
          ...
          await client.query(`UPDATE fee_installments SET invoice_id=$1,status='invoiced',branch_id=COALESCE(branch_id,$2) WHERE id=$3 AND invoice_id IS NULL`,[inv[0].id,branch,item.id]);
        }
        await queueFeeInvoiceNotifications(client,{schoolId:req.auth.schoolId,invoiceId:inv[0].id,studentId:sid,invoiceNo:inv[0].invoiceNo,amount:total,dueDate:invoiceDue});
  ...
  app.get('/api/fees/installments', authenticate, requireRoles(...financeRoles), async(req,res,next)=>{
    try{
      const {sessionId=null,studentId=null,status=null}=req.query;
      const params=[req.auth.schoolId,sessionId,req.auth.branchId||null,studentId,status];
      const {rows}=await pool.query(`SELECT fi.id,fi.student_id AS "studentId",s.admission_no AS "admissionNo",s.full_name AS "studentName",fi.session_id AS "sessionId",fi.fee_head_id AS "feeHeadId",fh.name AS "feeHeadName",fi.period_start AS "periodStart",fi.period_end AS "periodEnd",fi.due_date AS "dueDate",fi.amount,COALESCE(inv.paid_amount,0) AS "paidAmount",fi.invoice_id AS "invoiceId",fi.status FROM fee_installments fi JOIN students s ON s.id=fi.student_id JOIN fee_heads fh ON fh.id=fi.fee_head_id LEFT JOIN fee_invoices inv ON inv.id=fi.invoice_id AND inv.school_id=fi.school_id WHERE fi.school_id=$1 AND ($2::uuid IS NULL OR fi.session_id=$2) AND ($3::uuid IS NULL OR COALESCE(fi.branch_id,s.branch_id)=$3 OR s.branch_id IS NULL) AND ($4::uuid IS NULL OR fi.student_id=$4) AND ($5::text IS NULL OR fi.status=$5) ORDER BY fi.due_date,s.full_name`,params);
      res.json({installments:rows,totalDue:rows.filter(x=>x.status!=='paid'&&x.status!=='cancelled').reduce((s,x)=>s+Math.max(0,Number(x.amount||0)-Number(x.paidAmount||0)),0)});
    }catch(err){next(err)}
  });
}
