...
const { queueAttendanceNotifications } = require('./notifications');
...
function registerAttendanceRoutes(app, pool) {
  ...
  app.get('/api/attendance', authenticate, async (req,res,next) => {
    try {
      ...
      const { rows } = await pool.query(`SELECT a.id,a.student_id AS "studentId",s.admission_no AS "admissionNo",s.full_name AS "studentName",a.attendance_date AS "date",a.status,a.note,a.session_id AS "sessionId",a.branch_id AS "branchId",b.name AS "branchName",c.name AS "className",sec.name AS "sectionName" FROM attendance_records a JOIN students s ON s.id=a.student_id LEFT JOIN branches b ON b.id=a.branch_id LEFT JOIN enrollments e ON e.student_id=s.id AND e.session_id=a.session_id AND e.status='active' LEFT JOIN sections sec ON sec.id=e.section_id LEFT JOIN classes c ON c.id=sec.class_id WHERE a.school_id=$1 AND a.attendance_date=$2 AND ($3::uuid IS NULL OR a.session_id=$3) AND ($4::uuid IS NULL OR a.branch_id=$4 OR a.branch_id IS NULL) AND ($5::uuid IS NULL OR e.section_id=$5) ORDER BY c.name,sec.name,s.full_name`, [req.auth.schoolId,date,sessionId,req.auth.branchId || null,sectionId]);
  ...
  app.get('/api/attendance/roster', authenticate, requireRoles(...staffRoles), async (req,res,next) => {
    try {
      ...
      const teacherClause = req.auth.role === 'teacher' ? `AND EXISTS (SELECT 1 FROM teacher_subjects ts JOIN users tu ON tu.teacher_id=ts.teacher_id WHERE ts.school_id=$1 AND ts.section_id=e.section_id AND ts.session_id=e.session_id AND tu.id=$6 AND ($5::uuid IS NULL OR ts.branch_id=$5 OR ts.branch_id IS NULL))` : '';
      const { rows } = await pool.query(`SELECT s.id,s.admission_no AS "admissionNo",s.full_name AS "fullName",s.photo_url AS "photoUrl",e.session_id AS "sessionId",e.section_id AS "sectionId",sec.name AS "sectionName",c.id AS "classId",c.name AS "className",COALESCE(a.status,'present') AS status,a.note,a.id AS "attendanceId" FROM enrollments e JOIN students s ON s.id=e.student_id JOIN sections sec ON sec.id=e.section_id JOIN classes c ON c.id=sec.class_id LEFT JOIN attendance_records a ON a.school_id=e.school_id AND a.student_id=e.student_id AND a.attendance_date=$2 AND ($3::uuid IS NULL OR a.session_id=$3) AND ($5::uuid IS NULL OR a.branch_id=$5 OR a.branch_id IS NULL) WHERE e.school_id=$1 AND e.status='active' AND ($3::uuid IS NULL OR e.session_id=$3) AND ($4::uuid IS NULL OR e.section_id=$4) AND ($5::uuid IS NULL OR e.branch_id=$5 OR e.branch_id IS NULL OR c.branch_id=$5 OR c.branch_id IS NULL) AND (s.branch_id=$5 OR s.branch_id IS NULL OR $5::uuid IS NULL) ${teacherClause} ORDER BY c.name,sec.name,s.full_name`, params);
  ...
  app.get('/api/attendance/summary', authenticate, async (req,res,next) => {
    try {
      ...
      const { rows } = await pool.query(`SELECT a.student_id AS "studentId",s.admission_no AS "admissionNo",s.full_name AS "studentName",COUNT(*)::int AS total,COUNT(*) FILTER (WHERE a.status='present')::int AS present,COUNT(*) FILTER (WHERE a.status='absent')::int AS absent,COUNT(*) FILTER (WHERE a.status='late')::int AS late,COUNT(*) FILTER (WHERE a.status='half_day')::int AS "halfDay",COUNT(*) FILTER (WHERE a.status='leave')::int AS leave FROM attendance_records a JOIN students s ON s.id=a.student_id WHERE a.school_id=$1 AND a.attendance_date BETWEEN $2 AND $3 AND ($4::uuid IS NULL OR a.student_id=$4) AND ($5::uuid IS NULL OR a.branch_id=$5 OR a.branch_id IS NULL) GROUP BY a.student_id,s.admission_no,s.full_name ORDER BY s.full_name`, [req.auth.schoolId,from,to,studentId,req.auth.branchId || null]);
  ...
  app.post('/api/attendance/bulk', authenticate, requireRoles(...staffRoles), async (req,res,next) => {
    ...
    try {
      ...
      for (const item of records) {
        ...
        await queueAttendanceNotifications(client,{schoolId:req.auth.schoolId,studentId,date,status,note});
        results.push(rows[0]);
      }
