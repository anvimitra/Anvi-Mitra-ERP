...
function registerExamRoutes(app, pool) {
  ...
  app.get('/api/exams', authenticate, async (req,res,next) => {
    try {
      ...
      const teacherFilter = req.auth.role === 'teacher' ? `AND EXISTS (
        ...
        WHERE ts.school_id=e.school_id AND ts.session_id=e.session_id
          AND ($2::uuid IS NULL OR ts.branch_id=$2 OR ts.branch_id IS NULL)
      )` : '';
      const params=[req.auth.schoolId,branchId,sessionId||null,req.auth.sub,examTypeId||null];
      const { rows } = await pool.query(`SELECT e.id,e.session_id AS "sessionId",e.name,e.starts_on AS "startsOn",e.ends_on AS "endsOn",e.status,t.code AS "typeCode",t.name AS "typeName",e.branch_id AS "branchId" FROM exams e JOIN exam_types t ON t.id=e.exam_type_id WHERE e.school_id=$1 AND ($2::uuid IS NULL OR e.branch_id=$2 OR e.branch_id IS NULL) AND ($3::uuid IS NULL OR e.session_id=$3) AND ($5::uuid IS NULL OR e.exam_type_id=$5) ${teacherFilter} ORDER BY e.starts_on NULLS LAST,e.name`, params);
      res.json({ exams: rows });
    } catch(err){ next(err); }
  });
  ...
  app.post('/api/exams', authenticate, requireRoles(...staffRoles), async (req,res,next) => {
    try {
      ...
      if (req.auth.role === 'teacher') return res.status(403).json({ error: 'Teachers cannot create exams' });
      const branchId=req.auth.branchId||null;
  ...
  app.post('/api/exam-subjects', authenticate, requireRoles(...adminRoles), async (req,res,next) => {
    try {
      ...
      const { rows } = await pool.query(`WITH valid AS (
        SELECT e.id AS exam_id, e.session_id, c.id AS class_id, c.branch_id AS class_branch_id, s.id AS subject_id, e.branch_id AS exam_branch_id
      ...
      SELECT $1,CASE WHEN $5::uuid IS NULL THEN COALESCE(v.exam_branch_id,v.class_branch_id) ELSE $5 END,v.exam_id,v.subject_id,v.class_id,$6,$7,$8 FROM valid v
  ...
  app.post('/api/exam-marks', authenticate, requireRoles(...staffRoles), async (req,res,next) => {
    try {
      ...
      if (!rows.length) return res.status(403).json({ error: 'Student is not enrolled in the exam subject class, marks exceed maxMarks, or teacher is not assigned to this subject/class' });
