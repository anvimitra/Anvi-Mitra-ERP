...
function registerAdmissionRoutes(app, pool) {
  ...
  async function journal(client, auth, entityType, entityId, operation, payload) {
    await client.query(`INSERT INTO sync_changes(school_id,entity_type,entity_id,operation,payload,changed_by) VALUES($1,$2,$3,$4,$5::jsonb,$6)`, [auth.schoolId,entityType,entityId,operation,JSON.stringify(payload || {}),auth.sub]);
  }
  ...
  app.post('/api/admissions', authenticate, requireRoles(...officeRoles), async (req,res,next) => {
    try {
      ...
      await pool.query(`INSERT INTO sync_changes(school_id,entity_type,entity_id,operation,payload,changed_by) VALUES($1,'admission_application',$2,'create',$3::jsonb,$4)`, [req.auth.schoolId,rows[0].id,JSON.stringify(rows[0]),req.auth.sub]);
  ...
  app.post('/api/admissions/:id/status', authenticate, requireRoles(...officeRoles), async (req,res,next) => {
    try {
      ...
      await pool.query(`INSERT INTO sync_changes(school_id,entity_type,entity_id,operation,payload,changed_by) VALUES($1,'admission_application',$2,'update',$3::jsonb,$4)`, [req.auth.schoolId,rows[0].id,JSON.stringify(rows[0]),req.auth.sub]);
  ...
  app.post('/api/admissions/:id/enroll', authenticate, requireRoles(...officeRoles), async (req,res,next) => {
    ...
    try {
      ...
      await journal(client,req.auth,'student',student.rows[0].id,'create',{student:student.rows[0]});
      await journal(client,req.auth,'enrollment',enrollment.rows[0].id,'create',{enrollment:enrollment.rows[0],studentId:student.rows[0].id});
      await journal(client,req.auth,'admission_application',application.id,'update',{id:application.id,status:'enrolled',studentId:student.rows[0].id,enrollmentId:enrollment.rows[0].id});
  ...
  app.post('/api/students/:studentId/documents', authenticate, requireRoles(...officeRoles), async (req,res,next) => {
    try {
      ...
      await pool.query(`INSERT INTO sync_changes(school_id,entity_type,entity_id,operation,payload,changed_by) VALUES($1,'student_document',$2,'create',$3::jsonb,$4)`, [req.auth.schoolId,rows[0].id,JSON.stringify({...rows[0],studentId:req.params.studentId}),req.auth.sub]);
