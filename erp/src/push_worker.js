...
function getFirebaseApp() {
  if (firebaseReady) return admin.app();
...
async function processPushQueue(pool, batchSize=20) {
  ...
  try {
    ...
    for(const row of rows){
      try{
        const response=await admin.messaging().send({token:row.token,notification:{title:row.title,body:row.message},data:Object.fromEntries(Object.entries(row.data||{}).map(([k,v])=>[k,String(v??'')]))});
