...
function registerOrganizationRoutes(app, pool) {
    ...
    try {
      ...
      const { rows: branches } = await pool.query(
        `SELECT id,name,code,address,phone,email,logo_url AS "logoUrl",status,is_main AS "isMain"
         FROM branches WHERE school_id=$1 ORDER BY is_main DESC,name`, [req.auth.schoolId]);
  ...
  app.patch('/api/branches/:id', authenticate, requireRoles(...schoolRoles), async (req, res, next) => {
    try {
      ...
      const { rows } = await pool.query(
        `UPDATE branches SET ${sets.join(',')},updated_at=now()
         WHERE id=$${vals.length-1} AND school_id=$${vals.length}
         RETURNING id,name,code,address,phone,email,logo_url AS "logoUrl",status,is_main AS "isMain"`, vals);
  ...
  app.get('/api/platform/schools', authenticate, requireRoles(...platformRoles), async (_req, res, next) => {
    try {
      const { rows } = await pool.query(`SELECT s.id,s.name,s.code,s.status,s.created_at AS "createdAt",
        ...
        FROM schools s LEFT JOIN school_settings ss ON ss.school_id=s.id
        LEFT JOIN mobile_app_configs mac ON mac.school_id=s.id ORDER BY s.name`);
  ...
  app.patch('/api/platform/schools/:id', authenticate, requireRoles(...platformRoles), async (req, res, next) => {
    ...
    try {
      const b = req.body || {};
      const schoolMap = {name:'name',code:'code',status:'status'};
      ...
      const make = map => {
        const set=[], vals=[];
        for (const [key,column] of Object.entries(map)) if (Object.prototype.hasOwnProperty.call(b,key)) {
          set.push(`${column}=$${vals.length+1}`);
          vals.push(key==='code' ? String(b[key]).trim().toUpperCase() : key==='appSlug' ? String(b[key]).trim().toLowerCase() : b[key] ?? null);
        }
        return {set,vals};
      };
      ...
      if (err.code === '23505') return res.status(409).json({error:'School code or app identifier is already in use'});
      next(err);
  ...
  app.patch('/api/platform/schools/:id/branding', authenticate, requireRoles(...platformRoles), async (req,res,next) => {
    try {
      const b=req.body||{};
      const allowed={logoUrl:'logo_url',primaryColor:'primary_color',secondaryColor:'secondary_color',displayName:'display_name'};
      const set=[],vals=[];
      for(const [key,column] of Object.entries(allowed)) if(Object.prototype.hasOwnProperty.call(b,key)){set.push(`${column}=$${vals.length+1}`);vals.push(b[key]??null);}
      if(!set.length)return res.status(400).json({error:'No branding fields supplied'});
      vals.push(req.params.id);
      const {rows}=await pool.query(`UPDATE school_settings SET ${set.join(',')},updated_at=now() WHERE school_id=$${vals.length} RETURNING school_id AS "schoolId",display_name AS "displayName",logo_url AS "logoUrl",primary_color AS "primaryColor",secondary_color AS "secondaryColor"`,vals);
      if(!rows.length)return res.status(404).json({error:'School settings not found'});
  ...
  app.post('/api/platform/schools', authenticate, requireRoles(...platformRoles), async (req,res,next) => {
    ...
    try {
      const b=req.body||{};
      const name=String(b.name||'').trim(), code=String(b.code||'').trim().toUpperCase(), displayName=String(b.displayName||name).trim();
      const appName=String(b.appName||displayName).trim(), appSlug=String(b.appSlug||code.toLowerCase()).trim().toLowerCase();
