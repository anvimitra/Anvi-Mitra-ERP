...
function registerOrganizationRoutes(app, pool) {
  ...
  app.patch('/api/platform/schools/:id/branding',authenticate,requireRoles(...platformRoles),async(req,res,next)=>{
    ...
    try{
      ...
      const settings=[]; const values=[];
      for(const [key,column] of Object.entries(allowed)) if(Object.prototype.hasOwnProperty.call(b,key)){settings.push(column+'=authenticate,requireRoles(...platformRoles),async(req,res,next)=>{
    const client=await pool.connect();
    try{
...
+(values.length+1));values.push(b[key]??null)}
      if(!settings.length)return res.status(400).json({error:'No branding fields supplied'});
      values.push(req.params.id);
      await client.query('BEGIN');
      const exists=await client.query('SELECT id FROM schools WHERE id=$1',[req.params.id]);
      if(!exists.rows.length){await client.query('ROLLBACK');return res.status(404).json({error:'School not found'})}
      const updated=await client.query('UPDATE school_settings SET '+settings.join(',')+',updated_at=now() WHERE school_id=authenticate,requireRoles(...platformRoles),async(req,res,next)=>{
    const client=await pool.connect();
    try{
      const b=req.body||{},sf={name:'name',code:'code',status:'status'},stf={displayName:'display_name',logoUrl:'logo_url',primaryColor:'primary_color',secondaryColor:'secondary_color',address:'address',phone:'phone',email:'email',website:'website',timezone:'timezone',currencyCode:'currency_code',locale:'locale',dateFormat:'date_format'},af={appName:'app_name',appSlug:'app_slug',androidPackage:'android_package',iosBundleId:'ios_bundle_id',apiBaseUrl:'api_base_url',appLogoUrl:'logo_url',appPrimaryColor:'primary_color',appSecondaryColor:'secondary_color',supportEmail:'support_email',supportPhone:'support_phone',minAppVersion:'min_app_version',forceUpdate:'force_update',appStatus:'status'};
      const make=(obj)=>{const set=[],vals=[];for(const[k,c]of Object.entries(obj))if(Object.prototype.hasOwnProperty.call(b,k)){set.push(`${c}=$${vals.length+1}`);vals.push(k==='code'?String(b[k]).trim().toUpperCase():k==='appSlug'?String(b[k]).trim().toLowerCase():b[k]??null)}return{set,vals}};
...
module.exports={registerOrganizationRoutes};
+values.length+' RETURNING school_id AS "schoolId",display_name AS "displayName",logo_url AS "logoUrl",primary_color AS "primaryColor",secondary_color AS "secondaryColor"',values);
      if(b.logoUrl!==undefined||b.primaryColor!==undefined||b.secondaryColor!==undefined){
        const mobile=[];const mv=[];
        const mm={logoUrl:'logo_url',primaryColor:'primary_color',secondaryColor:'secondary_color'};
        for(const [key,column] of Object.entries(mm))if(Object.prototype.hasOwnProperty.call(b,key)){mobile.push(column+'=authenticate,requireRoles(...platformRoles),async(req,res,next)=>{
    const client=await pool.connect();
    try{
      const b=req.body||{},sf={name:'name',code:'code',status:'status'},stf={displayName:'display_name',logoUrl:'logo_url',primaryColor:'primary_color',secondaryColor:'secondary_color',address:'address',phone:'phone',email:'email',website:'website',timezone:'timezone',currencyCode:'currency_code',locale:'locale',dateFormat:'date_format'},af={appName:'app_name',appSlug:'app_slug',androidPackage:'android_package',iosBundleId:'ios_bundle_id',apiBaseUrl:'api_base_url',appLogoUrl:'logo_url',appPrimaryColor:'primary_color',appSecondaryColor:'secondary_color',supportEmail:'support_email',supportPhone:'support_phone',minAppVersion:'min_app_version',forceUpdate:'force_update',appStatus:'status'};
      const make=(obj)=>{const set=[],vals=[];for(const[k,c]of Object.entries(obj))if(Object.prototype.hasOwnProperty.call(b,k)){set.push(`${c}=$${vals.length+1}`);vals.push(k==='code'?String(b[k]).trim().toUpperCase():k==='appSlug'?String(b[k]).trim().toLowerCase():b[k]??null)}return{set,vals}};
...
module.exports={registerOrganizationRoutes};
+(mv.length+1));mv.push(b[key]??null)}
        if(mobile.length){mv.push(req.params.id);await client.query('UPDATE mobile_app_configs SET '+mobile.join(',')+',updated_at=now() WHERE school_id=authenticate,requireRoles(...platformRoles),async(req,res,next)=>{
    const client=await pool.connect();
    try{
      const b=req.body||{},sf={name:'name',code:'code',status:'status'},stf={displayName:'display_name',logoUrl:'logo_url',primaryColor:'primary_color',secondaryColor:'secondary_color',address:'address',phone:'phone',email:'email',website:'website',timezone:'timezone',currencyCode:'currency_code',locale:'locale',dateFormat:'date_format'},af={appName:'app_name',appSlug:'app_slug',androidPackage:'android_package',iosBundleId:'ios_bundle_id',apiBaseUrl:'api_base_url',appLogoUrl:'logo_url',appPrimaryColor:'primary_color',appSecondaryColor:'secondary_color',supportEmail:'support_email',supportPhone:'support_phone',minAppVersion:'min_app_version',forceUpdate:'force_update',appStatus:'status'};
      const make=(obj)=>{const set=[],vals=[];for(const[k,c]of Object.entries(obj))if(Object.prototype.hasOwnProperty.call(b,k)){set.push(`${c}=$${vals.length+1}`);vals.push(k==='code'?String(b[k]).trim().toUpperCase():k==='appSlug'?String(b[k]).trim().toLowerCase():b[k]??null)}return{set,vals}};
...
+mv.length,mv)}
      ...
      await client.query('COMMIT');res.json({branding:updated.rows[0]||null,message:'Branding updated'});
