...
function providerConfigured() { return Boolean(process.env.SMS_API_URL && process.env.SMS_API_KEY); }

function buildSmsPayload(row) { return { to: row.phone, message: row.message, sender: process.env.SMS_SENDER_ID || undefined, eventType: row.event_type, clientReference: row.id }; }
...
async function sendSms(row) {
  ...
  try {
    const response = await fetch(process.env.SMS_API_URL,{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${process.env.SMS_API_KEY}`},body:JSON.stringify(buildSmsPayload(row)),signal:controller.signal});
...
async function processSmsQueue(pool,batchSize=25){
  ...
  try{
    ...
    for(const row of rows){
      ...
      try{const providerMessageId=await sendSms(row);await client.query(`UPDATE notification_sms_queue SET status='sent',provider_message_id=$2,sent_at=now(),last_error=NULL WHERE id=$1`,[row.id,providerMessageId]);processed++}
      catch(err){await client.query(`UPDATE notification_sms_queue SET status='failed',last_error=$2 WHERE id=$1`,[row.id,String(err.message).slice(0,1000)])}
    }
    await client.query('COMMIT'); return {processed,skipped:false};
...
function istWeekKey(date=new Date()){
  const p=istDateParts(date); const d=new Date(Date.UTC(Number(p.year),Number(p.month)-1,Number(p.day))); const day=d.getUTCDay();
  const monday=new Date(d); monday.setUTCDate(d.getUTCDate()-(day===0?6:day-1));
...
async function queueWeeklyFeeReminders(pool){
  ...
  try{
    await client.query('BEGIN'); const weekKey=istWeekKey();
    const schools=await client.query(`SELECT id FROM schools WHERE status IS NULL OR status='active'`); let queued=0;
    for(const school of schools.rows){
      ...
      for(const parent of rows){
        const due=parent.dueDate?` Earliest due date: ${parent.dueDate}.`:''; const message=`Fee reminder for ${parent.studentName}: outstanding balance ₹${Number(parent.balance).toFixed(2)}.${due} Please pay the pending fee.`;
        ...
        if(parent.phone) await client.query(`INSERT INTO notification_sms_queue(school_id,user_id,student_id,phone,message,event_type) VALUES($1,$2,$3,$4,$5,'weekly_fee_reminder')`,[school.id,parent.userId,parent.studentId,parent.phone,message]); queued++;
      }
    }
    await client.query('COMMIT'); return {queued,weekKey};
  }catch(err){await client.query('ROLLBACK').catch(()=>{});throw err}finally{client.release()}
}

function startNotificationWorker(pool){
  if(!pool||process.env.NOTIFICATION_WORKER_ENABLED==='false') return null; let running=false;
  const tick=async()=>{if(running)return;running=true;try{await processSmsQueue(pool);const p=istDateParts();const day=new Intl.DateTimeFormat('en-US',{timeZone:'Asia/Kolkata',weekday:'short'}).format(new Date());const hour=Number(new Intl.DateTimeFormat('en-US',{timeZone:'Asia/Kolkata',hour:'2-digit',hour12:false}).format(new Date()));if((day==='Sat'||day==='Sun')&&hour>=9&&hour<12)await queueWeeklyFeeReminders(pool)}catch(err){console.error('notification worker:',err.message)}finally{running=false}};
  tick(); return setInterval(tick,5*60*1000);
}
