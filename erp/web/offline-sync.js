window.AnviOfflineSync=(()=>{
  ...
  async function pull(request,deviceKey){
    if(!navigator.onLine)return {pulled:0,offline:true};
    const cursor=Number((await get('sync_cursor'))||0);
    const d=await request('/api/sync/changes?cursor='+encodeURIComponent(cursor)+'&limit=200&deviceKey='+encodeURIComponent(deviceKey));
    const changes=d.changes||[];
    if(changes.length)await put('sync_cursor',Number(d.cursor||cursor));
    for(const change of changes)await cache('sync-change',String(change.cursor),change);
    return {pulled:changes.length,cursor:Number(d.cursor||cursor),hasMore:Boolean(d.hasMore)};
  }
  async function flush(request,deviceKey){
    ...
    const baseCursor=Number((await get('sync_cursor'))||0);
    const d=await request('/api/sync/push',{method:'POST',body:JSON.stringify({deviceKey,changes:q.map(x=>({...x,baseCursor:x.baseCursor??baseCursor}))})});
  ...
  async function syncNow(){
    if(!boundRequest||!boundDeviceKey)return {sent:0,pulled:0,remaining:(await outbox()).length,offline:!navigator.onLine};
    if(busy)return {sent:0,pulled:0,remaining:(await outbox()).length,busy:true};
    busy=true;
    try{
      await registerDevice(boundRequest,boundDeviceKey);
      const pushed=await flush(boundRequest,boundDeviceKey);
      const pulled=await pull(boundRequest,boundDeviceKey);
      return {...pushed,...pulled};
    }catch(error){window.dispatchEvent(new CustomEvent('anvi:sync-error',{detail:error}));return {sent:0,pulled:0,remaining:(await outbox()).length,offline:false,error};}
    finally{busy=false;}
  }
