...
CREATE INDEX IF NOT EXISTS class_teacher_assignments_teacher_idx
  ON class_teacher_assignments(school_id,teacher_id,session_id);
CREATE INDEX IF NOT EXISTS class_teacher_assignments_section_idx
  ON class_teacher_assignments(school_id,section_id,session_id);
...
CREATE INDEX IF NOT EXISTS parent_push_queue_pending_idx
  ON parent_push_queue(status,queued_at);
CREATE INDEX IF NOT EXISTS parent_push_queue_school_idx
  ON parent_push_queue(school_id,status,queued_at);
...
FOR EACH ROW
WHEN (NEW.marked_by IS NOT NULL)
...
DECLARE
  ...
  event_data JSONB;
BEGIN
  IF TG_OP='UPDATE' AND OLD.status IS NOT DISTINCT FROM NEW.status AND OLD.note IS NOT DISTINCT FROM NEW.note THEN
    RETURN NEW;
  END IF;
  ...
  message_text := format('Attendance update: %s (%s) is %s on %s.',
    COALESCE(student_name,'Student'), COALESCE(admission_no,'N/A'), replace(NEW.status,'_',' '), NEW.attendance_date);
  ...
  LOOP
    ...
    FROM parents pr
    WHERE pr.school_id=NEW.school_id AND pr.user_id=p.user_id AND pr.phone IS NOT NULL;
    ...
    FROM parent_device_tokens dt
    WHERE dt.school_id=NEW.school_id AND dt.user_id=p.user_id AND dt.is_active=true;
  END LOOP;
...
FOR EACH ROW
EXECUTE FUNCTION notify_parents_after_attendance();
